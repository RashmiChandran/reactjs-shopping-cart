      ...
      {productItem.isAddedtoCart ? (
        <div className="flex grid-cols-2 gap-2 grid w-4/5">
          <div className="flex items-center">
           <button  onClick={()=> decrementItem(productItem)} disabled={productItem.count <= 1 } className="disabled:opacity-50 disabled:cursor-text"><FaMinusSquare className="w-5 h-5" /> </button>
            <span className="mx-2">{productItem.count}</span>
            {productItem.count >= 1 ? <button onClick={()=>incrementItem(productItem)}><FaPlusSquare  className="w-5 h-5"/></button> : null}
          </div>
